...
def update_form(form_id):
    ...
    data = request.get_json(silent=True) or {}
    for key in ("title", "description", "season", "fee", "is_active"):
        if key in data:
...
def submit_registration():
    ...
    reg = Registration(
        form_id=data["form_id"],
        player_user_id=data["player_user_id"],
        parent_user_id=data.get("parent_user_id", g.user["id"]),
...
def get_registration(reg_id):
    ...
    return api_response.success(reg.to_dict())
...
def update_registration_status(reg_id):
    ...
    if status == "approved" and old_status != "approved":
        ...
        form = RegistrationForm.query.get(reg.form_id)
        if form:
    ...
    return api_response.success(reg.to_dict(), "Registration status updated.")
...
def upload_waiver(reg_id):
    ...
    if not file_path:
        return api_response.bad_request("file_path is required.")
