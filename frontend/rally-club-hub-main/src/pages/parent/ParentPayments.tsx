  ...
  return (
    <div className="mt-3 border-t pt-3 space-y-2">
      ...
      {payments.map((p: any) => {
        ...
        return (
          <div key={p.id} className={`flex items-center justify-between text-sm rounded px-3 py-2 ${installmentOverdue ? "bg-red-50 border border-red-200" : "bg-muted/50"}`}>
            <div>
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge variant="secondary" className={statusColor[inv.status] || ""}>{overdue ? "overdue" : inv.status}</Badge>
            </div>
              ...
              <Card key={inv.id}>
                <CardContent className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {inv.player && <span className="text-xs text-muted-foreground">{inv.player.full_name}</span>}
                  </div>
