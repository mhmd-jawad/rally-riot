...
export default function CoachAttendance() {
  ...
  const getAttendanceStatus = (playerId: number) => {
    const rec = attendance.find((a: any) => a.player_user_id === playerId);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {selectedEvent && (
        <Card>
          <CardHeader><CardTitle>RSVPs & Attendance</CardTitle></CardHeader>
          <CardContent>
            {rsvps.length === 0 ? (
              <p className="text-sm text-muted-foreground">No RSVPs for this event yet</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead><tr className="border-b text-left text-sm text-muted-foreground">
                    <th className="p-3">Player</th><th className="p-3">RSVP</th><th className="p-3">Attendance</th><th className="p-3">Actions</th>
                  </tr></thead>
                  <tbody>
                    {rsvps.map((rsvp: any) => {
                      const attStatus = getAttendanceStatus(rsvp.player_user_id);
                      return (
                        <tr key={rsvp.id} className="border-b last:border-0 hover:bg-muted/50">
                          <td className="p-3 font-medium text-sm">{rsvp.player?.full_name || `Player #${rsvp.player_user_id}`}</td>
                          <td className="p-3">
                            <div className="flex items-center gap-1">
                              {rsvpStatusIcon[rsvp.status]}
                          ...
                          <td className="p-3 flex gap-2">
                            <Button size="sm" variant={attStatus === "present" ? "default" : "outline"}
                              onClick={() => markMutation.mutate({ playerId: rsvp.player_user_id, status: "present" })}>
                              Present
                            </Button>
                            <Button size="sm" variant={attStatus === "absent" ? "destructive" : "outline"}
                              onClick={() => markMutation.mutate({ playerId: rsvp.player_user_id, status: "absent" })}>
                              Absent
                            </Button>
                          </td>
                        </tr>
